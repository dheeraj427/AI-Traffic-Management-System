...
if "last_ui_update" not in st.session_state: st.session_state.last_ui_update = 0.0
if "siren_playing_now" not in st.session_state: st.session_state.siren_playing_now = False
...
if "draw_commands" not in st.session_state: st.session_state.draw_commands = []
if "lane_counts" not in st.session_state: st.session_state.lane_counts = {lane: 0 for lane in LANES}
if "detailed_counts" not in st.session_state:
    st.session_state.detailed_counts = {lane: {"car": 0, "motorcycle": 0, "bus": 0, "truck": 0} for lane in LANES}
if "active_lane" not in st.session_state: st.session_state.active_lane = None
if "forced_lane" not in st.session_state: st.session_state.forced_lane = None
if "weather_status" not in st.session_state: st.session_state.weather_status = "Clear"
if "smoothed_lane_counts" not in st.session_state: st.session_state.smoothed_lane_counts = {lane: 0.0 for lane in LANES}
...
# ==========================================
def speak(text):
    if "audio_enabled" in st.session_state and not st.session_state.audio_enabled: return
    try:
...
def add_bgm():
    ...
    if os.path.exists(bgm_file):
        with open(bgm_file, "rb") as f: b64 = base64.b64encode(f.read()).decode()
...
if st.session_state.audio_enabled: add_bgm()
...
if st.session_state.get("running", False) and "siren_thread" not in st.session_state:
...
def send_telegram_alert(message, force=False):
    if not st.session_state.get("alerts_enabled", False): return
    current_time = time.time()
    if not force and (current_time - st.session_state.last_telegram_time < 15): return
    st.session_state.last_telegram_time = current_time
    st.toast("Sending Alert...", icon="🚨")
    try:
        requests.post(f"https://api.telegram.org/bot{BOT_TOKEN}/sendMessage", json={"chat_id": CHAT_ID, "text": f"🚨 ASTRA:\n{message}"}, timeout=3)
    except: pass
...
def calculate_lane_times(lane_counts, rl_active, current_state):
    ...
    if rl_active and total_cars > 0:
        ...
        for lane in LANES: durations[lane] = max(MIN_GREEN, durations[lane] + modifier)
    return durations
    ...
    for lane, col in zip(LANES, cols):
        with col:
            ...
            if is_active and isinstance(lane_wait[lane], int):
                ...
                if total_duration > 0: percentage = max(0, min(100, (lane_wait[lane] / total_duration) * 100))
...
with st.sidebar:
    ...
    if app_mode == "Live AI Feed":
        ...
        st.caption("*(Visually detects an ambulance and instantly clears its specific lane while halting others)*")

        st.markdown("---")
        st.session_state.alerts_enabled = st.toggle("🔕 Enable Telegram Alerts", value=False)
...
# ==========================================
if app_mode == "Live AI Feed":
    default_src = "traffic.mp4" if os.path.exists("traffic.mp4") else "https://github.com/intel-iot-devkit/sample-videos/raw/master/car-detection.mp4"
    camera_url = st.text_input("Video Source", default_src)
    ...
    if st.session_state.running:
        ...
        while st.session_state.running:
            ...
            if frame_counter % 2 == 0: st.session_state.cap.grab()
            ...
            if frame_counter % 7 == 0:
                if st.session_state.siren_trigger_event:
                    ...
                    send_telegram_alert("🚑 AMBULANCE SIREN DETECTED! Initiating Emergency Preemption.", force=True)
                ...
                if frame_counter % 21 == 0:
                    ...
                    if is_foggy:
                        ...
                        yolo_conf = 0.15
                        env_indicator.warning("🌫️ Bad Weather Detected: Lowering YOLO confidence threshold.")
                    else: env_indicator.empty()
                ...
                with torch.no_grad(): results = model(frame, size=640)
                ...
                d_counts = {lane: {"car": 0, "motorcycle": 0, "bus": 0, "truck": 0} for lane in LANES}
                ...
                for _, det in detections.iterrows():
                    ...
                    for lane, (lx1, ly1, lx2, ly2) in lanes_region.items():
                        if lx1 <= cx < lx2 and ly1 <= cy <= ly2:
                            l_counts[lane] += 1
                            if v_class in d_counts[lane]: d_counts[lane][v_class] += 1

                alpha = 0.20
                for l in LANES:
                    st.session_state.smoothed_lane_counts[l] = (alpha * l_counts[l]) + ((1 - alpha) * st.session_state.smoothed_lane_counts[l])
                ...
                def trigger_siren_audio():
                    audio_file = "siren.mp3" if os.path.exists("siren.mp3") else ("siren.m4a" if os.path.exists("siren.m4a") else None)
                    if audio_file:
                        with open(audio_file, "rb") as f: b64_siren = base64.b64encode(f.read()).decode()
                        with siren_placeholder.container():
                            st.error("🚨 EMERGENCY SIREN BROADCAST ACTIVE")
                            if st.session_state.audio_enabled:
                                st.markdown(f'<iframe src="data:audio/mpeg;base64,{b64_siren}" allow="autoplay" style="display:none" id="siren-frame"></iframe>', unsafe_allow_html=True)
                    else:
                        with siren_placeholder.container(): st.error("🚨 EMERGENCY ACTIVE (Missing siren audio file!)")
                ...
                if detected_ambulance_lane:
                    ...
                    if not st.session_state.siren_playing_now:
                        ...
                        send_telegram_alert(f"🚑 VISUAL AMBULANCE DETECTED IN {LANE_NAMES[active_lane].upper()}!", force=True)
                    trigger_siren_audio()
                elif s_sim_on and s_active_bool:
                    forced_lane = "EMERGENCY"
                    active_lane = "EMERGENCY"
                    if not st.session_state.siren_playing_now: st.session_state.siren_playing_now = True
                    trigger_siren_audio()
                else:
                ...
                if current_time >= st.session_state.signal_end_time:
                    current_state = traffic_status(sum(st.session_state.lane_counts.values()))
                    ...
                    st.session_state.lane_durations = calculate_lane_times(st.session_state.lane_counts, rl_enabled, current_state)
                ...
                st.session_state.track_history = {k: v for k, v in st.session_state.track_history.items() if k in active_ids}

                draw_cmds = []
                for objectID, centroid in objects.items():
                    cx, cy = centroid[0], centroid[1]
                    if objectID not in st.session_state.track_history: st.session_state.track_history[objectID] = []

                    if frame_counter % 6 == 0: st.session_state.track_history[objectID].append((cx, cy, current_time))
                    if len(st.session_state.track_history[objectID]) > 20: st.session_state.track_history[objectID].pop(0)
                    ...
                    if len(history) == 20:
                        ...
                        if dist < 10 and (current_time - stime) > 3.0:
                            if int_x1 < cx < int_x2 and int_y1 < cy < int_y2: is_anomaly = True

                    is_violation = False
                    if int_x1 < cx < int_x2 and int_y1 < cy < int_y2 and active_lane != "None" and active_lane is not None:
                        if len(history) > 5:
                            prev_x, prev_y, _ = history[-5]
                            origin_lane = None
                            if prev_y < int_y1 and int_x1 < prev_x < 500: origin_lane = "Lane 1"
                            elif prev_y > int_y2 and 500 < prev_x < int_x2: origin_lane = "Lane 2"
                            elif prev_x < int_x1 and int_y1 < prev_y < int_y2: origin_lane = "Lane 4"
                            elif prev_x > int_x2 and int_y1 < prev_y < int_y2: origin_lane = "Lane 3"
                    ...
                    draw_cmds.append({"id": objectID, "cx": cx, "cy": cy, "color": color, "alert": alert_text})
            ...
            if frame_counter % 14 == 0:
                ...
                cv2.rectangle(annotated, (int_x1, int_y1), (int_x2, int_y2), (0, 255, 255), 2)
                cv2.putText(annotated, "INTERSECTION ZONE", (int_x1, int_y1 - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 2)

                for cmd in st.session_state.draw_commands:
                    cx, cy, color, alert = cmd["cx"], cmd["cy"], cmd["color"], cmd["alert"]
                    cv2.putText(annotated, f"ID {cmd['id']}", (cx - 10, cy - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
                    cv2.circle(annotated, (cx, cy), 4, color, -1)
                    if alert: cv2.putText(annotated, alert, (cx - 30, cy - 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 2)

                for _, det in st.session_state.cached_detections.iterrows():
                    if ambulance_demo and det["name"] in ["truck", "bus"] and st.session_state.forced_lane:
                        cv2.putText(annotated, "AMBULANCE", (int(det.xmin), int(det.ymin) - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
                    else:
                ...
                _, buffer = cv2.imencode(".jpg", annotated, [cv2.IMWRITE_JPEG_QUALITY, 25])
            ...
            if current_time - st.session_state.last_ui_update >= 1.5:
                ...
                display_counts = {k: int(round(v)) for k, v in st.session_state.smoothed_lane_counts.items()}
                total = sum(display_counts.values())
                level = traffic_status(total)

                fig, ax = plt.subplots(figsize=(8, 3))
                ax.bar(list(display_counts.keys()), list(display_counts.values()), color="#00c6ff")
                ax.set_title("Traffic Density", color="white")
                fig.patch.set_facecolor("#0f172a")
                ax.set_facecolor("#0f172a")
                ax.tick_params(colors="white")
                max_val = max(display_counts.values()) if display_counts.values() else 0
                ...
                with lights_placeholder.container():
                    traffic_lights_with_timers(current_time, st.session_state.cycle_schedule, override_active_lane=st.session_state.forced_lane)

                with status_panel.container():
                    flash_class = "emergency-flash" if st.session_state.forced_lane else ""
                    ...
                    cols[3].metric("Engine Mode", "CLOUD OPTIMIZED")

                    if ambulance_demo and st.session_state.forced_lane and st.session_state.forced_lane not in ["EMERGENCY", "PEDESTRIAN"]:
                        st.error(f"🚨 VISUAL AMBULANCE OVERRIDE: {LANE_NAMES.get(st.session_state.forced_lane, st.session_state.forced_lane).upper()} CLEARED.")
                    elif st.session_state.forced_lane == "EMERGENCY":
                        st.error("🚨 SIREN DETECTED: ALL LANES STOPPED. CLEARING INTERSECTION.")
                ...
                with lane_placeholder.container():
                    lane_breakdown_ui(st.session_state.detailed_counts, st.session_state.active_lane)
...
# ==========================================
    ...
    df_violations = pd.read_sql_query("SELECT * FROM violations ORDER BY timestamp DESC LIMIT 100", conn)
    ...
    df_traffic = pd.read_sql_query("SELECT * FROM traffic_logs_v2 ORDER BY timestamp DESC LIMIT 100", conn)
