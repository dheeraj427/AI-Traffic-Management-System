...
if "detailed_counts" not in st.session_state: st.session_state.detailed_counts = {lane: {"car": 0, "motorcycle": 0, "bus": 0, "truck": 0} for lane in LANES}
...
with st.spinner("🤖 ASTRA AI Core Initializing... Loading High-Accuracy GPU Network..."):
    model = load_model()
...
def send_telegram_alert(message, force=False):
    ...
    if not force and (current_time - st.session_state.last_telegram_time < 15): 
        return 
    ...
    if st.session_state.running:
        ...
        while st.session_state.running:
            ...
            if frame_counter % 3 == 0: 
                ...
                if ambulance_demo:
                    h, w, _ = frame.shape
                    lanes_region_temp = {"Lane 1":(0,0,w//2,h//2), "Lane 2":(w//2,0,w,h//2), "Lane 3":(0,h//2,w//2,h), "Lane 4":(w//2,h//2,w,h)}
                ...
                def trigger_siren_audio():
                    ...
                    if audio_file:
                        ...
                        with siren_placeholder.container():
                            ...
                            if st.session_state.audio_enabled:
                                st.markdown(
                                    f'<iframe src="data:audio/mpeg;base64,{b64_siren}" allow="autoplay" style="display:none" id="siren-frame"></iframe>', 
                                    unsafe_allow_html=True
                                )
                    else:
                        with siren_placeholder.container():
                            st.error("🚨 EMERGENCY ACTIVE (Missing siren.mp3 or siren.m4a file!)")
            ...
            if frame_counter % 5 == 0:
                ...
                for cmd in st.session_state.draw_commands:
                    ...
                    if alert:
                        cv2.putText(annotated, alert, (cx - 30, cy - 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 2)
                ...
                for _, det in st.session_state.cached_detections.iterrows():
                    if ambulance_demo and det["name"] in ['truck', 'bus'] and st.session_state.forced_lane:
                        box_color = (0, 0, 255) 
                        cv2.putText(annotated, "AMBULANCE", (int(det.xmin), int(det.ymin) - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.8, box_color, 2)
                    else:
                        box_color = (0, 255, 255) 
                        cv2.rectangle(annotated, (int(det.xmin), int(det.ymin)), (int(det.xmax), int(det.ymax)), box_color, 2)
            ...
            if current_time - st.session_state.last_ui_update >= 1.5:
                ...
                with status_panel.container():
                    ...
                    if ambulance_demo and st.session_state.forced_lane and st.session_state.forced_lane != "EMERGENCY" and st.session_state.forced_lane != "PEDESTRIAN":
                    ...
                    elif st.session_state.forced_lane == "EMERGENCY": st.error("🚨 SIREN DETECTED: ALL LANES STOPPED. CLEARING INTERSECTION.")
                    elif st.session_state.forced_lane == "PEDESTRIAN": st.warning("🚶‍♂️ PEDESTRIAN CROSSING ACTIVE.")
                ...
                log_entry = {"Timestamp": current_dt, "Total_Vehicles": total, "Status": level, "Weather": st.session_state.weather_status}
                for l in LANES:
                    log_entry[f"{l}_Total"] = st.session_state.lane_counts[l]
                    for v in ["car", "motorcycle", "bus", "truck"]:
                        log_entry[f"{l}_{v}"] = st.session_state.detailed_counts[l][v]
                
                st.session_state.full_data_log.append(log_entry)

                if current_time - st.session_state.last_status_time >= 30 and not st.session_state.forced_lane:
                    ...
                    log_to_db('traffic_logs_v2', (current_dt, total, level, st.session_state.weather_status))
